...
import { DateToUTDate, GetFormmatterForCurrency } from '@/lib/helpers';
import { UserSettings } from '@prisma/client';
import { useQueries, useQuery } from '@tanstack/react-query';
import { icons, TrendingDown, TrendingUp, Wallet } from 'lucide-react';
...
function StatsCards ({from,to,userSettings}: Props) {
    const  statsQuery = useQuery<GetBalanceStatsResponseType>({
        ...
        queryFn:() =>
            fetch(
                `/api/stats/balance?from=${DateToUTDate(from)}&to=${DateToUTDate(to)}`
            ).then((res) => res.json())
    })
 ...
 />
      ...
      <SkeletonWrapper isLoading={statsQuery.isFetching}>
        ...
        icon={
           <TrendingDown
           className=' h-12 w-12 items-center rounded-lg p-2 text-red-500 bg-red-400/10 '
           />
        }
        />
             </SkeletonWrapper>
